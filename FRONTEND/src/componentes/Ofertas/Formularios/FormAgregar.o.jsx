                    ...
                    <Form data-bs-theme={isDarkMode ? "dark" : "light"} className={`w-11/12 h-5/6 border-4 overflow-y-auto px-5 pt-9 flex flex-col justify-start items-center sm:w-3/5 xl:w-1/2 ${isDarkMode ? `bg-dark-greenMedio  border-white `:`bg-gradient-to-b to-green-100 from-blue-200 border-black`}`} encType="multipart/form-data">
                        ...
                        <div className={`w-full font-textos flex flex-col gap-6 ${isDarkMode ? `text-white` : `text-black`}`}>
                            <div className={`relative `}>
                                ...
                                <h4 className={`text-center text-xl mb-4`}>Agregar Oferta</h4>
                            </div>
        
                            <Form.Group className="mb-3" controlId="titulo">
                                <Form.Label className="text-lg">Título</Form.Label>
                                <Form.Control required type="text" name="titulo" placeholder="Especial de Navidad 20% de Descuento" value={nuevaOferta.titulo} onChange={handleInputChange} className={` placeholder:text-sm placeholder:text-gray-200 ${isDarkMode ? `bg-gray-400 border-white focus:bg-gray-400 text-base p-2`: `border-black`}`}/>
                            </Form.Group>
        
                            <Form.Group className="mb-3" controlId="descripcion">
                                <Form.Label className="text-lg">Descripción</Form.Label>
                                <Form.Control required type="text" name="descripcion" placeholder="Para estas navidades..." value={nuevaOferta.descripcion} onChange={handleInputChange} className={` placeholder:text-sm placeholder:text-gray-200 ${isDarkMode ? `bg-gray-400 border-white focus:bg-gray-400 text-base p-2`: `border-black`}`}/>
                            </Form.Group>
        
                            <Form.Group className="mb-3" controlId="fecha_expiracion">
                                <Form.Label className="text-lg">Fecha de Expiración</Form.Label>
                                <Form.Control required type="date" name="fecha_expiracion" value={nuevaOferta.fecha_expiracion} onChange={handleInputChange} className={` placeholder:text-sm placeholder:text-gray-200 ${isDarkMode ? `bg-gray-400 border-white focus:bg-gray-400 text-base p-2`: `border-black`}`}/>
                            </Form.Group>
