const usuarioModel = require('../models/usuarios.m')
...
class accederControllers {
    async verificar(req, res, next) {
        ...
        console.log(acceder);
        try {
            if (!acceder.Correo || !acceder.Contrasena) {
                throw new ErrorDetallado('Datos de entrada incorrecto o faltante',400)
            }
            const db = await usuarioModel.find({'correo':acceder.Correo});
            console.log(db);
            if (db.length < 1) {
    ...
    async registrar (req, res, next){
        ...
        try {
            if (!registrar.Nombre || !registrar.Apellido || !registrar.Cedula || !registrar.Correo  || !registrar.Contrasena  || !registrar.ConfirmarContrasena) {
                throw new ErrorDetallado('Datos de entrada incorrecto o faltante',400)
            }
            ...
            const crear = {rol:rol,nombre:registrar.Nombre,apellido:registrar.Apellido,contrasena:encriptado,cedula:registrar.Cedula,correo:registrar.Correo}
